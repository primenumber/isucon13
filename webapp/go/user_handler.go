...
func getIconHandler(c echo.Context) error {
	...
	var icon Icon
	if err := tx.GetContext(ctx, &icon, "SELECT image_path,icon_hash FROM users inner join icons as i on users.id = i.user_id WHERE name = ?", username); err != nil {
...
func postIconHandler(c echo.Context) error {
	...
	rs, err := tx.ExecContext(ctx, "INSERT INTO icons (user_id, image_path, icon_hash) VALUES (?, ?, ?)", userID, iconPath, iconHashStr)
	if err != nil {
...
func fillUserResponse(ctx context.Context, tx *sqlx.Tx, userModel UserModel) (User, error) {
	...
	if err := tx.GetContext(ctx, &icon, "SELECT image_path, icon_hash FROM icons WHERE user_id = ?", userModel.ID); err != nil {
		if !errors.Is(err, sql.ErrNoRows) {
			return User{}, err
		}
		iconHashStr = fallbackImageHash
	} else {
		iconHashStr = icon.IconHash
	}
