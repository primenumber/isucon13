...
func getUserStatisticsHandler(c echo.Context) error {
	...
	query := "select `rank` from (select react.id, react.name, react.react + tips.tip as score, rank() over (order by react.react + tips.tip desc, react.name) as `rank` from (select def.id, def.name, (def.react + IFNULL(rs.react, 0)) as react from  (select id, name, 0 as react from users u) as def left join (select u.id, count(*) as react  FROM users u INNER JOIN livestreams l ON l.user_id = u.id INNER JOIN reactions r ON r.livestream_id = l.id group by u.id) as rs on def.id = rs.id) as react inner join (select u.id, IFNULL(SUM(l2.tip), 0) as tip FROM users u INNER JOIN livestreams l ON l.user_id = u.id INNER JOIN livecomments l2 ON l2.livestream_id = l.id group by u.id) as tips on react.id = tips.id order by `rank` desc) as T  where id = ?"
